...
class DataGenerator(object):
    def __init__(
        ...
        self._feat_dir = self._feat_cls.get_normalized_feat_dir_mel_mod()

        self._filenames_list = list()
        self._nb_frames_file = 0     # Using a fixed number of frames in feat files. Updated in _get_label_filenames_sizes()
        self._nb_mel_bins1 = self._feat_cls.get_nb_mel_bins1()
        self._nb_mel_bins2 = self._feat_cls.get_nb_mel_bins2()
    ...
    def get_data_sizes(self):
       ...
       # print('get data feat_shape, label_shape', feat_shape, label_shape)
        ...
        while 1:
            ...
            if self._is_eval:
                for i in range(self._nb_total_batches):
                  ...
                  #  print('inside eval feat', feat.shape)
                    yield feat
                   ...
                   # print('i',i)
                    ...
                    while len(self._circ_buf_feat) < self._feature_batch_seq_len:
                       ...
                       # print('temp_feat', temp_feat.shape)
                        ...
                        # If self._per_file is True, this returns the sequences belonging to a single audio recording
                        if self._per_file:
                           # print('Boom, here it goes inside')
                            ...
                            #print('feat_extra_frames',feat_extra_frames)
                            #print('extra_feat', extra_feat.shape)
                            #print('temp_label.shape[0]', temp_label.shape[0])
                            label_extra_frames = self._label_batch_seq_len - temp_label.shape[0]
                            #print('label_extra_frames', label_extra_frames)
                            ...
                            for l_row in extra_labels:
                              #  print('l_row', l_row)
                                self._circ_buf_label.append(l_row)
                       # print('This code is not going inside per file mode')
                        file_cnt = file_cnt + 1
