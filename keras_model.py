...
def get_model(data_in, data_out, dropout_rate, nb_cnn2d_filt, f_pool_size, t_pool_size,
    ...
    print('data shapes',data_in[-3], data_in[-2], data_in[-1])
    print('sepct_start', spec_start)
    # CNN
    spec_cnn = spec_start
    ...
    if is_accdoa:
        ...
        if doa_objective is 'mse':
            ...
            model.compile(optimizer=optimizer1, loss=['binary_crossentropy', 'mse'], loss_weights=weights)
            ...
            model.compile(optimizer=optimizer1, loss=['binary_crossentropy', masked_mse], loss_weights=weights)
        else:
