...
def dump_DCASE2021_results(_data_gen, _feat_cls, _dcase_output_folder, _sed_pred, _doa_pred):
    ...
    #print('_sed_pred.shape[0]',_sed_pred.shape[0])
    for file_cnt in range(_sed_pred.shape[0] // frames_per_file):
        ...
        #print('frames_per_file',frames_per_file)
        #print('dc',dc)
...
def get_accdoa_labels(accdoa_in, nb_classes):
    #print('nb_classes',nb_classes, accdoa_in.shape)
    x, y, z = accdoa_in[:, :, :nb_classes], accdoa_in[:, :, nb_classes:2*nb_classes], accdoa_in[:, :, 2*nb_classes:]
    print('distance for threshold', np.sqrt(x**2 + y**2 + z**2))
    sed_out1 = np.sqrt(x**2 + y**2 + z**2)
    sed = np.sqrt(x**2 + y**2 + z**2) > 0.04
   # print('get_accdoa_labels' , sed, accdoa_in.shape) 
    return sed, accdoa_in, sed_out1
...
def main(argv):
    ...
    for split_cnt, split in enumerate(test_splits):
        ...
        for epoch_cnt in range(nb_epoch):
            ...
            print('data_gen_train.generate()', data_gen_train.generate())
            ...
            print('tr_loss[epoch_cnt]',tr_loss[epoch_cnt])
            # predict once per epoch
            pred = model.predict_generator(
                generator=data_gen_val.generate(),
                steps=5 if params['quick_test'] else data_gen_val.get_total_batches_in_data(),
                verbose=1
            )
            print('pred', pred.shape)
            #df_file=pd.DataFrame(pred)
            #df_file.to_csv('/home/amrgaballah/Desktop/filenam.csv', header=None, index= None)
            if params['is_accdoa']:
                sed_pred, doa_pred, sed_out11 = get_accdoa_labels(pred, nb_classes)
                #print('sed_pred, doa_pred', sed_pred, doa_pred)
                sed_pred = reshape_3Dto2D(sed_pred)
                doa_pred = reshape_3Dto2D(doa_pred)
                sed_out11 = reshape_3Dto2D(sed_out11)
                print('after reshaping sed_pred, doa_pred', sed_pred.shape, doa_pred.shape)
                df_file=pd.DataFrame(sed_out11)
                df_file.to_csv('/home/amrgaballah/Desktop/filenam1.csv', header=None, index= None)
                df_file1=pd.DataFrame(doa_pred)
                df_file1.to_csv('/home/amrgaballah/Desktop/filenam2.csv', header=None, index= None)
            else:
